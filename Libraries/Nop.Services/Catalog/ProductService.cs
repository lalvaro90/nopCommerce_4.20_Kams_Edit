...
namespace Nop.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        public void ApplyClientDiscount(ref List<Product> products)
        {
            if (_workContext.CurrentCustomer.Discount > 0)
            {
                var percentage = (decimal)(_workContext.CurrentCustomer.Discount) / 100;
                var val = Convert.ToDecimal(percentage);
                foreach (var p in products)
                {
                    p.Price = p.Price - (p.Price * val);
                }
            }
        }

        public void ApplyClientDiscount(ref Product products)
        {
            if (_workContext.CurrentCustomer.Discount > 0)
            {
                var percentage = (decimal)(_workContext.CurrentCustomer.Discount) / 100;
                ...
                products.Price = products.Price - (products.Price * val);

            }
        }


        public void ValidateProductPrice(ref List<Product> products)
        {
            var priceList = _workContext.CurrentCustomer.PriceList;
            if (_workContext.CurrentCustomer.PriceList > 0)
            {
        ...
        public void validateProductPrice(ref Product product)
        {
            var priceList = _workContext.CurrentCustomer.PriceList;
            if (priceList > 0)
            {
        ...
        public virtual IList<Product> GetAllProductsDisplayedOnHomepage()
        {
            ...
            var priceList = _workContext.CurrentCustomer.PriceList;
            ValidateProductPrice(ref products);
        ...
        public virtual Product GetProductById(int productId)
        {
            ...
            var product = _cacheManager.Get(key, () => _productRepository.GetById(productId));
            validateProductPrice(ref product);

            ApplyClientDiscount(ref product);
        ...
        public virtual IList<Product> GetProductsByIds(int[] productIds)
        {
            ...
            ApplyClientDiscount(ref sortedProducts);
        ...
        /// <summary>
        /// Updates all products visible price
        /// </summary>
        /// <param name="product">Product</param>
            ...
            int pageIndex = 0, int pageSize = int.MaxValue)
        {
            ...
            ValidateProductPrice(ref prods);

            var products = new PagedList<Product>(prods, pageIndex, pageSize);
            return products;
        }
        ...
        public virtual IList<Product> GetCrosssellProductsByShoppingCart(IList<ShoppingCartItem> cart, int numberOfProducts)
        {
            ...
            ValidateProductPrice(ref result);
